...
function parseArgs() {
  ...
  const cmdParts = cmdIndex === -1 ? argv.filter(a => !a.startsWith('--timeout=') && !a.startsWith('--idle-ms=')) : argv.slice(cmdIndex + 1);

  if (cmdParts.length === 0) {
    console.error('Usage: node run-with-timeout.mjs [--timeout=MS] [--idle-ms=MS] -- <command...>');
    process.exit(2);
  }
...
const resetIdle = () => { lastOutput = Date.now(); };

child.stdout.on('data', (chunk) => { resetIdle(); process.stdout.write(chunk); });
child.stderr.on('data', (chunk) => { resetIdle(); process.stderr.write(chunk); });
...
const idleCheck = setInterval(() => {
  if (Date.now() - lastOutput > idleMs) {
    idleTimedOut = true;
    console.error(`[run-with-timeout] No output for ${Math.round(idleMs/1000)}s — killing process`);
    try { child.kill('SIGTERM'); } catch (e) { /* ignore */ }
  }
...
child.on('exit', (code, signal) => {
  ...
  if (idleTimedOut) {
    console.error(`[run-with-timeout] Process killed after idle timeout (${elapsed}s)`);
    process.exit(125);
  }
  if (timedOut) {
