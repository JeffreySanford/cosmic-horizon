...
function parseArgs() {
  ...
  if (!opts.cmd || opts.cmd.length === 0) {
    console.error(
      'Usage: node scripts/nx-retry.cjs [--retries=N] [--delay=ms] -- <command...>',
    );
    process.exit(2);
  }
  return opts;
...
async function main() {
  ...
  for (let attempt = 1; attempt <= opts.retries; attempt++) {
    if (attempt > 1)
      console.log(`Retry attempt ${attempt}/${opts.retries} for: ${cmd}`);
    ...
    if (attempt < opts.retries) {
      console.log(
        `Command failed with exit ${code}. Waiting ${opts.delay}ms before retry.`,
      );
