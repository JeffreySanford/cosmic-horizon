...
if (!/MVP Scope Lock/i.test(content)) {
  console.error(
    'SCOPE-LOCK.MD does not appear to contain an MVP Scope Lock header',
  );
  process.exit(1);
}
...
if (!fs.existsSync(charterPath)) {
  console.error(
    'Product charter missing (required) at documentation/product/PRODUCT-CHARTER.MD',
  );
  process.exit(1);
}
...
if (!/Product Charter/i.test(charterContent)) {
  console.error(
    'PRODUCT-CHARTER.MD does not appear to contain a Product Charter header',
  );
  process.exit(1);
}
