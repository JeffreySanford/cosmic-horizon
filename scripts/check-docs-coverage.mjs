...
const codeExt = ['.ts', '.js', '.jsx', '.tsx', '.py', '.go', '.java', '.rb', '.sh'];
const docExt = ['.md'];
...
if (codeFiles.length > 0 && docFiles.length === 0) {
  console.error('Documentation coverage check failed: code files staged but no markdown changes.');
  console.error('  Staged code files:');
  codeFiles.forEach((f) => console.error('    ' + f));
  console.error('Please update/write documentation related to your changes before pushing.');
  process.exit(1);
}
