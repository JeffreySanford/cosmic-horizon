...
export class LoggingService implements OnModuleDestroy {
  ...
  constructor() {
    const enabled =
      (process.env['LOGS_REDIS_ENABLED'] ?? 'false').toLowerCase() === 'true';
    if (!enabled) return;
    ...
    try {
      ...
      this.logger.warn(
        `LoggingService Redis disabled: ${(err as Error)?.message ?? 'unknown'}`,
      );
      this.redis = null;
    }
  }
  ...
  async add(entry: LogOptions): Promise<void> {
    ...
    if (this.redis) {
      try {
        ...
        this.logger.warn(
          `Redis log write failed: ${(err as Error)?.message ?? 'unknown'}`,
        );
        this.redis = null;
      }
    }
  ...
  async getRecent(limit = 100, offset = 0): Promise<LogEntry[]> {
    if (this.redis) {
      try {
        ...
        this.logger.warn(
          `Redis log read failed: ${(err as Error)?.message ?? 'unknown'}`,
        );
        this.redis = null;
      }
    }
