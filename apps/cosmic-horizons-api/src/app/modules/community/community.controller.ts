import { Body, Controller, Get, Post, Patch, Param, Query, HttpCode, HttpStatus, Logger, UseGuards, Request, ForbiddenException } from '@nestjs/common';
...
export class CommunityController {
  ...
  async approvePost(@Request() req: AuthenticatedRequest, @Param('id') id: string) {
    if (req.user?.role !== 'admin' && req.user?.role !== 'moderator') {
      throw new ForbiddenException('Insufficient permissions');
    }

    const approved = await this.communityService.approveDiscovery(id, req.user?.id ?? null);
  ...
  async hidePost(@Request() req: AuthenticatedRequest, @Param('id') id: string) {
    if (req.user?.role !== 'admin' && req.user?.role !== 'moderator') {
      throw new ForbiddenException('Insufficient permissions');
    }

    const result = await this.communityService.hideDiscovery(id, req.user?.id ?? null);
