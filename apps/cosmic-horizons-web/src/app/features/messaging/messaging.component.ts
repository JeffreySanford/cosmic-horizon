...
export class MessagingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private updateSiteStatusCounts(siteId: string) {
    ...
    const operational = siteElements.filter(
  ...
  getActiveElementCount(): number {
    return this.elements.filter((element) => element.status === 'operational')
      .length;
  }
  ...
  getMessagingStatusLabel(): string {
    ...
    const rabbit = this.liveStats.infra.rabbitmq.connected
      ? 'RabbitMQ up'
      : 'RabbitMQ down';
    const kafka = this.liveStats.infra.kafka.connected
      ? 'Kafka up'
      : 'Kafka down';
    return `${rabbit} | ${kafka}`;
  }
  ...
  private updateChartDataOnly() {
    ...
    this.svg.selectAll('.node circle').attr('fill', (d: any) => {
  ...
  private initChart() {
    ...
    this.simulation = d3
      ...
      .force(
        ...
        d3
          .forceCollide<any>()
          .radius((d: any) => (d.type === 'site' ? 25 : 15))
          .strength(0.8),
      )
      .force(
        'center',
        d3.forceCenter(this.width / 2, this.height / 2).strength(0.3),
      )
      .velocityDecay(0.4);
  }

  private updateChart() {
    if (!this.svg || this.sites.length === 0 || this.elements.length === 0)
      return;
    ...
    const nodes = [
      ...this.sites.map((s) => ({
        ...s,
        type: 'site',
        isCentral: s.id === this.centralSiteId,
      })),
    ...
    const siteLinks = this.sites.flatMap((sourceSite, sourceIndex) =>
      this.sites.slice(sourceIndex + 1).map((targetSite) => ({
    ...
    nodeEnter
      .append('text')
      .attr('dy', (d: any) =>
        d.type === 'site' ? (d.isCentral ? 35 : 25) : 15,
      )
