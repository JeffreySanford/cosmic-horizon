...
test('no Angular NG0912 duplicate component-definition warnings on app load', async ({ page }) => {
  const logs: string[] = [];
...
test('labeling center should not emit Aladin logger CORS/fetch or passive-event warnings', async ({ page }) => {
  const logs: string[] = [];
  page.on('console', (msg) => logs.push(msg.text()));
  ...
  const joined = logs.join('\n');
  expect(joined).not.toMatch(/AladinLiteLogger|Failed to fetch|Unable to preventDefault/i);
});
