...
test.fixme(
  'community post requiring moderation is hidden until approved',
  async ({ page, request }) => {
    ...
    expect(feedJsonDirect.some((f: any) => f.title === payload.title)).toBe(
      true,
    );
      ...
      .poll(
        async () =>
          await page.evaluate((t) => {
            ...
            if (!ng || !el || typeof ng.getComponent !== 'function')
              return false;
            try {
              // runtime-only browser helper (Playwright/Angular devtools)
...
test.fixme(
  'admin can hide a visible community post and UI updates accordingly',
  async ({ page, request }) => {
